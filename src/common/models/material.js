...
module.exports = function(Material) {
    ...
    Material.doPut = function (req, cb) {
        ...
        req.forEach(function(item) {
            cnt++;
            console.log('Processing request item['+cnt+']: id:' + item.id + ', kmat:' + item.kmat + ', mvm:' + item.mvm + ', hmotnost:' + item.hmotnost + ', mnozstvi:' + item.mnozstvi);
            ...
            .then(function(inst) {
                ...
                respCnt++;
                console.log('respCnt: ' + respCnt + 'resp so far: ' + JSON.stringify(Material.resp));
        ...
        return new Promise(function(resolve, reject) {
            ...
            switch (workflow) {
                ...
                case sc.FLOW_CS:
                    ...
                    .then(function(inst) {
                        kafka.sendEventP(inst.id, inst.kmat, null, inst.mvm, inst.hmotnost, inst.mnozstvi, inst);
                        resolve(inst);
                    });
                    break;
                case sc.FLOW_FUS:
                ...
                case sc.FLOW_FLUCS:
                    ...
                    .then(function(inst) {
                        if (insertData) { // optionally perform create step
                            return Material.createNew(insertData.kmat, insertData.mvm, insertData.hmotnost, insertData.mnozstvi);
                        } else { return inst; }    
                    })
                    .then(function(inst) {
